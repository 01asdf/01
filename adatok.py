...
def arguments(args):
    ...
    return args
def init():
    data.dict_users= {i: np.array([]) for i in range(data.number_users)}
    #Adatok ellenőrzése
    data.data_are_correct = is_data_correct()


    if data.config_number==0:
        data.user_labels_percents=labeling.labeling(data.number_users)
    else:
        ...
        if len(data.user_labels_percents) < data.number_users:
    ...
    if data.secure_aggregation:
        for i in range(1, 2 ** data.number_users):
